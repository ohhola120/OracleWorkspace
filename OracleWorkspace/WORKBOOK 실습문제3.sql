...
SELECT CLASS_NAME, (SELECT DEPARTMENT_NAME FROM TB_DEPARTMENT WHERE TB_CLASS.DEPARTMENT_NO = 
TB_DEPARTMENT.DEPARTMENT_NO) NAME
FROM TB_CLASS;
...
-- 9.
-- WHERE S.CLASS_NAME = '인문사회'
SELECT CLASS_NAME,PROFESSOR_NAME
FROM TB_CLASS S
JOIN TB_CLASS_PROFESSOR T ON T.CLASS_NO = S.CLASS_NO
JOIN TB_PROFESSOR R USING(PROFESSOR_NO)
...
WHERE CATEGORY = '예체능' AND PROFESSOR_NO IS NULL; -- 일치하는 행이 0개, 1 ~ N개
...
WHERE DEPARTMENT_NAME = '서반아어학과'; -- PROFESSOR_NO IS NULL;
